...
describe('dir_tree tool', () => {
  ...
  it('returns tree data for provided path', async () => {
    ...
    expect(asMock(getDirectoryTree)).toHaveBeenCalledWith('/root', {
      maxDepth: 10,
      includeHidden: false,
    });
  });
  ...
  it('passes provided options to getDirectoryTree', async () => {
    asMock<typeof getDirectoryTree>(getDirectoryTree).mockResolvedValueOnce({
      path: '/root',
      name: 'root',
      size: 0,
    ...
    await dirTree.handler({ maxDepth: 2, includeHidden: true }, context);
    expect(asMock(getDirectoryTree)).toHaveBeenCalledWith('/root', {
      maxDepth: 2,
      includeHidden: true,
    });
